...
def test_show_phone(empty_address_book, arguments, expected):
    """Перевіряє show_phone для різних аргументів.

    Дано:
        Книга з JohnDoe та параметризовані аргументи ``show_phone``.
    Коли:
        Викликається ``show_phone``.
    Тоді:
        Повертається очікуваний номер або повідомлення про помилку.

    Args:
        empty_address_book: Книга з JohnDoe.
